...
const initDatabase = async () => {
  ...
  if (!existingAdmin) {
    ...
    const isMatch = bcrypt.compareSync('admin123', existingAdmin.password_hash);
    if (!isMatch) {
      const passwordHash = bcrypt.hashSync('admin123', 10);
      await dbRun('UPDATE administrators SET password_hash = ? WHERE email = ?', [passwordHash, adminEmail]);
    }
  }
